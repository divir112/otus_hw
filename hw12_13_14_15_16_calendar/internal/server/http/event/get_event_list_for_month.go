...
func (s *EventServiceHTTP) GetEventListForMonth(w http.ResponseWriter, r *http.Request) {
	...
	events, err := s.app.GetEventsByDate(ctx, 30, requestModel.Date)
	if err != nil {
	...
	_, err = w.Write(resp)
	if err != nil {
		s.logger.Error("can't write response get events list for month: ", err)
	}
}
